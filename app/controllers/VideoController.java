...
public class VideoController extends Controller {
    ...
    public static Result searchByPage(String content, Integer page){
        ...
        try {
            String key = "videosearch" + content + "page";
            jedis = jedisPool.getResource();
            if (!jedis.exists(key)){
                int statuscode = new HttpUtils().sendCacheRequest(baseUrl + "/cache/video/search/page/" + page + "?search=" + URLEncoder.encode(content, "utf-8"));
                if (statuscode == 200){
    ...
    public static Result getSideBarVideos(Integer id){
        Jedis jedis = null;
        try {
            String key = "videosidebar" + id;
            jedis = jedisPool.getResource();
            if (!jedis.exists(key)){
                int statuscode = new HttpUtils().sendCacheRequest(baseUrl + "/cache/video/sidebar/" + id);
                if (statuscode == 200){
